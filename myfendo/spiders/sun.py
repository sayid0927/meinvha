...
class SunSpider(CrawlSpider):
    ...
    def book_list(self, response):
        ...
        for link in links:
            yield scrapy.Request(link,meta={'book_List': book_List,'book_list_tag':book_list_tag}, callback=self.book_href)

    def book_href(self, response):
        links = response.xpath('//div[@class="b-oper"]//@href').extract()[0]
        book_List = response.meta['book_List']
        book_list_tag = response.meta['book_list_tag']
        if not links:
            yield scrapy.Request(links, meta={'href_url': links, 'book_List': book_List,'book_list_tag':book_list_tag}, callback = self.book_dir)
    ...
    def book_dir(self, response):
        ...
        for link in links:
            i = links.index(link)
            yield scrapy.Request(href_url + "/" + link, meta={'bookname': bookname, 'booktitle': booktitle[i], 'bookdirnumber':i, 'book_List': book_List,'book_list_tag':book_list_tag}, callback=self.process_item)
